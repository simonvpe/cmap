...
[source,c++]
----
constexpr auto make_lock(int key, int value) // -> (fn(int) -> optional<int>)
{
    return [key,value](auto _key) -> optional<int>
    {
...
[source,c++]
----
static_assert( nullopt == my_lock(84) );
...
[source,c++]
----
constexpr auto make_branch(auto left, auto right) // -> (fn -> optional<int>)
{

    return [left,right](int key) -> optional<int>
    {
